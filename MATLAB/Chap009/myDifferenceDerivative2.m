...
% 前進/後退/中央差分による差分微分
f_forward = conv(f, [1; -1; 0], 'valid');
f_forward = f_forward ./ h;
f_backward = conv(f, [0; 1; -1], 'valid');
f_backward = f_backward ./ h;
f_central = conv(f, [1; 0; -1], 'valid');
f_central = f_central ./ (2*h);
% プロット
figure;
